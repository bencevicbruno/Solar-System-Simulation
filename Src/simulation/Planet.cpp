...
Planet::Planet(std::string name, float radius, float mass, float rotationSpeed, float revolutionSpeed, float distanceFromSun) :
	Planet(name, radius, mass, rotationSpeed, revolutionSpeed, distanceFromSun, 0.0f, Math::CreateRandom(0.0f, 360.0f),
...
Planet::Planet(std::string name, float radius, float mass, float rotationSpeed, float revolutionSpeed, 
	float distanceFromSun, float rotationAngle, float revolutionAngle, Texture texture, SphereModel model) :
	name(name),
	radius(radius),
	mass(mass),
...
{
	return Vector::FromPolar(distanceFromSun, revolutionAngle, 0.0f);
}
...
void Planet::render() const
{
	...
	const ShaderProgram& shaderProgram = ShaderRegistry::GetShader("object");
	...
	Vector position = Vector::FromPolar(distanceFromSun, revolutionAngle, 0.0f);
...
Planet* Planet::CreateRandom()
{
	...
	return new Planet(name, radius, mass, rotationSpeed, revolutionSpeed, 
		distanceFromSun, 0.0f, revolutionAngle, std::move(texture), std::move(model));
}